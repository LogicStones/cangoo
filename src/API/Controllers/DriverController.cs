...
namespace API.Controllers
{
    ...
    public class DriverController : BaseController
    {
        ...
        ////TBD: Move firebase related logic(s) to FireBaseController
                ...
                if (captain != null)
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        ...
                        if (responseString.Contains("access_token"))
                        {
                            ...
                            {
                                response.error = true;
                                response.message = ResponseKeys.captainBlocked; this.ApplicationID
                                    dic = new Dictionary<dynamic, dynamic>
                                                {
                            ...
                            var isCaptanInTrip = fc.isCaptainInTrip(captain.CaptainID.ToString());
                            DriverModel captainModel = new DriverModel
                            {
                                ...
                                priorityHourEndTime = captain.LastPriorityHourEndTime != null ? DateTime.Parse(captain.LastPriorityHourEndTime.ToString()).ToString(Common.dateFormat) : "",
        ...
        [HttpGet]
        public HttpResponseMessage getVehicleList([FromUri] GetVehicleListRequest model)
        {
            using (CangooEntities context = new CangooEntities())
            {
                FireBaseController fb = new FireBaseController();

                var onlineDrivers = fb.GetOnlineDrivers();
        ...
        [HttpPost]
        public HttpResponseMessage changeVehicleStatus([FromBody] ChangeVehicleStatusRequest model)
        {
            using (CangooEntities context = new CangooEntities())
            {
                FireBaseController fc = new FireBaseController();
                ...
                {
                    //If this endpoint is accessed then we have already confirmed that captain was not in trip. fc.isCaptainInTrip(vehcle.driverID) = false
                    LogOutAlreadyLoggedInCaptain(model.DeviceToken, model.driverID, cap.DeviceToken, fc.isCaptainInTrip(model.driverID));
                }
                ...
                fc.insertDeleteOnlineDriver(model.isBooked, model.driverID, model.vehicleID, Convert.ToInt32(vh.MakeID), vh.CompanyID.ToString(),
        ...
        public HttpResponseMessage logOutDriver([FromBody] VehicleDetail model)
        {
            if (model != null && !string.IsNullOrEmpty(model.driverID) && !string.IsNullOrEmpty(model.DeviceToken))
            {
                using (CangooEntities context = new CangooEntities())
                {
                    ...
                    {
                        FireBaseController fb = new FireBaseController();
                        fb.insertDeleteOnlineDriver(false, model.driverID, "", 0, "", "", "", "", "", null, "", "", "", "", "", "", "", 0, "", "", 0, "", "");

                        Common.OnlineOfflineDriver(model.driverID, model.vehicleID, false, Guid.Parse(ApplicationID));
                    }
        ...
        public async Task<HttpResponseMessage> resetPassword([FromBody] DriverModel model)
        {
            if (model != null && !string.IsNullOrEmpty(model.UserName))
            {
                ...
                if (cUser != null)
                {
                    ...
                    string newPassword = Common.RandomString();
                    ...
                    SendSMS.SendSms("Das Passwort für dein cangoo-Fahrer wurde nun zurückgesetzt.\nDein neues Passwort lautet " + newPassword, cUser.PhoneNumber);
        ...
        public HttpResponseMessage acceptLateBooking([FromBody] RequestModel model)
        {
                ...
                using (CangooEntities context = new CangooEntities())
                {
                    if (model.isCheckLaterBookingConflict)
                    {
                        ...
                        if (conf.isConflict)
                        {
                            dic = new Dictionary<dynamic, dynamic>
                                {
                                    { "alreadyBooked", DateTime.Parse(conf.pickUpDateTime).ToString(Common.dateFormat) }
                                };
                            response.data = dic;
                            response.error = false;
                    ...
                    var tp = context.Trips.Where(t => t.TripID.ToString() == model.tripID && t.TripStatusID == (int)App_Start.TripStatus.RequestSent && t.isLaterBooking == true).FirstOrDefault();
                    if (tp == null)
                    {
                        response.error = true;
                    ...
                    tp.TripStatusID = (int)App_Start.TripStatus.LaterBookingAccepted;
                    tp.CaptainID = new Guid(model.driverID);
                    context.SaveChanges();
                    FireBaseController fb = new FireBaseController();

                    //List<CanclReasonsModel> lstLaterBookingPassenger = Common.GetCancelReasons(context, false, true, false);
                    List<CanclReasonsModel> lstLaterBookingDriver = Common.GetCancelReasons(context, false, true, true);

                    string path = "Trips/" + model.tripID.ToString();
                    ...
                    {
                            /* REFACTOR
							 * Trip node deletion seems to be unnecessary.*/

                        await fb.delTripNode(tp.TripID.ToString());
                        fb.addRemovePendingLaterBookings(false, "", tp.TripID.ToString(), "", 0);
                        fb.addDeleteNode(false, lstLaterBookingDriver, "CancelReasonsLaterBooking/Captain");

                    });

                    //get upcoming later booking
                    Common.getUpcomingLaterBookingByDriverID(tp.CaptainID.ToString());
                    ...
                    * If we stop deleting trip node then following look up implementation will be no more needed. */
                    ...
                    if (!string.IsNullOrEmpty(resp.Body) && !resp.Body.Equals("null"))
                    {
                        ...
                        task = Task.Run(async () =>
                        {
                            await fb.updateDiscountTypeAndAmount(false, path, model.tripID.ToString(), temp["discount"]["amount"].ToString(), temp["discount"]["type"].ToString(), temp["isDispatchedRide"]);
                        });
                    }
        ...
        public HttpResponseMessage acceptRequest([FromBody] RequestModel model)
        {
            ...
            if (model != null && !string.IsNullOrEmpty(model.tripID) && model.isAccept && !string.IsNullOrEmpty(model.driverID) && !string.IsNullOrEmpty(model.vehicleID))
            {
                using (CangooEntities context = new CangooEntities())
                {
                    //Trip status for firebase
                    int bookingStatus = 0;

                    //Get trip current status before any status change
                    if (model.isLaterBooking)
                    {
                        bookingStatus = (int)App_Start.TripStatus.LaterBookingAccepted;
                    }
                    else
                    {
                        ...
                        if (model.isDispatchedRide.ToLower().Equals("true"))
                            bookingStatus = (int)App_Start.TripStatus.Accepted;
                        else if (model.isReRouteRequest)
                            bookingStatus = (int)App_Start.TripStatus.Cancel;
                        else
                            bookingStatus = (int)App_Start.TripStatus.RequestSent;
                    }
                    ...
                    if (model.isDispatchedRide.ToLower().Equals("true"))
                    {
                        ...
                        captainDeviceToken = context.Captains.Where(c => c.CaptainID.ToString().Equals(captainID)).FirstOrDefault().DeviceToken;
                    }

                    //In case of laterbooking the flag is not maintained appropriately

                    model.isWeb = trip.BookingModeID == (int)TripBookingMod.UserApplication ? false : true;

                    //Update trip status in database
                    var detail = context.spGetUpdateTripDataOnAcceptRide(model.tripID, model.driverID, (int)App_Start.TripStatus.Accepted,
                    model.vehicleID, bookingStatus, model.isLaterBooking == true ? 1 : 0, Common.getUtcDateTime()).FirstOrDefault();

                    if (detail != null)
                    {
                        List<CanclReasonsModel> lstcr = Common.GetCancelReasons(context, !model.isLaterBooking, model.isLaterBooking, true);
                        ...
                        AcceptRideDriverModel arm = new AcceptRideDriverModel
                        {
                            ...
                            laterBookingPickUpDateTime = Convert.ToDateTime(detail.PickUpBookingDateTime).ToString(Common.dateFormat),
                        ...
                        //For captain application state management. Send FCM to user.
                        if (fc.addTripToDriverNode(model.driverID, model.tripID, string.IsNullOrEmpty(model.distanceToPickUpLocation) ? "0" : model.distanceToPickUpLocation))
                        {
                            var task = Task.Run(async () =>
                            {
                                await fc.sendFCMToUserAfterAcceptRideRequest(model.driverID, detail.UserID.ToString(), arm, model.tripID, model.isWeb);
                            });
                            ...
                            if (model.isDispatchedRide.ToLower().Equals("true"))
                            {
                                ...
                                {
                                    await fc.sendNotificationsAfterDispatchingRide(captainDeviceToken, captainID, model.tripID);
                                });

                                //TBD: Log previous captain priority points
                                context.DispatchedRidesLogs.Add(new DispatchedRidesLog
                                {
                                    DispatchedBy = Guid.Parse(model.dispatcherID),
                                    CaptainID = Guid.Parse(captainID),
                                ...
                                if (!(bool)cap.IsPriorityHoursActive)
                                {
                                    ...
                                    fc.updateDriverEarnedPoints(captainID, cap.EarningPoints.ToString());
                                }
                            }
                        }
                        ...
                        if (model.isLaterBooking)
                        {
                            ...
                            fc.addDeleteNode(false, dic, "Trips/" + model.tripID + "/" + detail.UserID.ToString() + "/");
                        ...
                        Dictionary<string, string> dTrip = new Dictionary<string, string>
                            {
                                { "tripID", model.tripID }
                            };
                        fc.addDeleteNode(false, dTrip, "CustomerTrips/" + detail.UserID.ToString());
        ...
        private void LogOutAlreadyLoggedInCaptain(string NewDeviceToken, string DriverID, string ExistingDeviceToken, bool isAlreadyInTrip)
        {
            ...
            if (!NewDeviceToken.ToLower().Equals(ExistingDeviceToken.ToLower()))
            {
                ...
                if (!isAlreadyInTrip)
                {
                    ...
                    Common.OnlineOfflineDriver(DriverID, "", false, Guid.Parse(ApplicationID));
                }

                var task = Task.Run(async () =>
                {
                    await fc.sentSingleFCM(ExistingDeviceToken, dic, "cap_NewDeviceLoggedIn");
                });
            }
        }
        ...
        private LaterBookingConflict checkLaterBookingDate(string captainID, DateTime pickUpDateTime)
        {
            using (CangooEntities context = new CangooEntities())
            {
                ...
                var lstLaterTrips = context.Trips.Where(t => t.CaptainID.ToString() == captainID && t.TripStatusID == (int)App_Start.TripStatus.LaterBookingAccepted && t.isLaterBooking == true).ToList();
